/* =====================================================================
   Feedback_IA - Carga de la PAUTA GENERAL de evaluacion
   Base de datos: CobAuto

   Pauta unica aplicable a todas las carteras (aplica_todas = 1).
   Contenido derivado de la pauta vigente en codigo
   (app/services/mibanco_quality_pauta.py), aplicando las mismas
   generalizaciones que plantilla_general_evaluacion():
     - "Mibanco" y "Biznescob" -> "la entidad financiera"
     - PECC.1 pasa a llamarse "Respeto por la entidad y sus canales"
   Codigos, pesos y reglas de evaluacion se conservan sin cambios.
   ...
   para no alterar la nota de las llamadas ya evaluadas.

   COMO USARLO
     1. Ejecutar. Crea la pauta en BORRADOR (no publica).
     2. Revisar y publicar desde la pantalla de Pautas de evaluacion.
     3. Reanalizar una llamada y verificar que el JSON traiga esta pauta.
...
/* ------------------------- PARAMETROS ------------------------- */
DECLARE @nombre         NVARCHAR(160) = N'Pauta general de evaluacion';
DECLARE @descripcion    NVARCHAR(MAX) = N'Pauta base de monitoreo de calidad aplicable a todas las carteras. Migrada desde la definicion en codigo.';
DECLARE @version        INT           = 1;
DECLARE @aplica_todas   BIT           = 1;   -- aplica a todas las carteras
DECLARE @grupo_nombre   NVARCHAR(120) = N'General';
...
/* Con aplica_todas = 1 no se registran carteras de alcance. */
...
INSERT INTO CobAuto.dbo.CRM_IA_PAUTA_CRITERIO
    (id_pauta, id_bloque, codigo_criterio, nombre, tipo_criterio, peso, detalle,
     ...
     recomendacion, orden, activo)
VALUES
    (@id_pauta, @id_bloque, N'PECC.1', N'Respeto por la entidad y sus canales',
     'PUNTUABLE', 5.0, N'Protege la imagen de la entidad financiera',
     N'No desacredita a la entidad financiera, a sus colaboradores, áreas, procesos o canales; tampoco responsabiliza a terceros para justificar una mala gestión.',
     N'Cumple cuando se refiere a la entidad financiera, sus personas, canales y procesos de forma profesional y sin desacreditarlos.',
     N'No cumple cuando desacredita a la entidad financiera, sus colaboradores, procesos o canales, o culpa a terceros para justificar una mala gestión.',
     N'Aplica a toda referencia del asesor sobre la entidad financiera, sus canales, colaboradores o procesos.',
     N'ERROR_CRITICO_CUMPLIMIENTO', N'TRANSCRIPCION',
     1, 0,
     NULL, 9, 1);
...
VALUES
    (@id_pauta, @id_bloque, N'PENC.1', N'Saludo de bienvenida',
     ...
     N'Saluda al inicio de la interacción, indicando nombre y apellidos del ejecutivo, sin omitir que actúa en representación de la entidad financiera.',
     N'Cumple cuando inicia con saludo, se identifica y comunica que actúa en representación de la entidad financiera.',
...
PRINT 'Pauta general creada en estado BORRADOR.';
...
/* Peso por bloque: PECUF 30, PECN 30, PECC 30, PENC 10 */
...
/* Verificar que no quedaron menciones a marcas */
SELECT codigo_criterio, nombre
FROM CobAuto.dbo.CRM_IA_PAUTA_CRITERIO
WHERE id_pauta = @id_pauta
  AND (nombre LIKE '%Mibanco%' OR nombre LIKE '%Biznescob%'
    OR detalle LIKE '%Mibanco%' OR detalle LIKE '%Biznescob%'
    OR regla_evaluacion LIKE '%Mibanco%' OR regla_evaluacion LIKE '%Biznescob%'
    OR regla_cumple LIKE '%Mibanco%' OR regla_cumple LIKE '%Biznescob%'
    OR regla_no_cumple LIKE '%Mibanco%' OR regla_no_cumple LIKE '%Biznescob%');
